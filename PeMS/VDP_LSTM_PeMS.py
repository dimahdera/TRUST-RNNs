...
def main_function(time_step=144, input_dim=963, units=400, class_num=7 , batch_size=10, epochs=600, kl_factor=0.001,
     lr = 0.0001, lr_end=0.000001 , Random_noise=True, gaussain_noise_std=0.3, Adversarial_noise=True, Targeted=False, epsilon=0.007, adversary_target_cls=3,
     BIM_adversarial=True, alpha= 1, maxAdvStep=100,
     Training=False, Testing=True, continue_training=False, saved_model_epochs=100):

    PATH = './VDP_lstm5/vdp_saved_models_units_{}_lr_{}/vdp_lstm_epoch_{}/'.format(units,lr, epochs) 
...
#                                  for grad in gradients]  
    ...
    if Training:
        wandb.init(entity = "dimah", project="VDP_LSTM_PEMS_units_{}_epochs_{}_lr_{}".format(units, epochs, lr)) 
        if continue_training:
            saved_model_path = './VDP_lstm4/vdp_saved_models_units_{}_lr_{}/vdp_lstm_epoch_{}/'.format(units,lr, saved_model_epochs)             
        ...
        if (epochs > 1):
            ...
            plt.ylim(0, 1.1)
            plt.title("VDP LSTM on PEMS Data")
            plt.xlabel("Epochs")
            plt.ylabel("Accuracy")
            plt.legend(loc='lower right')
            plt.savefig(PATH + 'VDP_LSTM_on_PEMS_Data_acc.png')
            plt.close(fig)
            ...
            plt.ylabel("Error")
            plt.legend(loc='lower right')
            plt.savefig(PATH + 'VDP_LSTM_on_PEMS_Data_error.png')
            plt.close(fig)
