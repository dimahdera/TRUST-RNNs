...
def main_function(time_step=1, input_dim=140, units=200, class_num=5 , batch_size=50, epochs=100, kl_factor=0.0001, lr = 0.001, lr_end=0.00001 , 
     Random_noise=False, gaussain_noise_std=0.5, Adversarial_noise=True, Targeted=False, epsilon=0.3, adversary_target_cls=3,
     BIM_adversarial=True, alpha=1, maxAdvStep=100,
     Training=False, Testing=False, continue_training=False, saved_model_epochs=0):

    PATH = './VDP_gru/vdp_saved_models_units_{}/vdp_gru_epoch_{}/'.format(units, epochs)     
...
#                                  for grad in gradients]  
    ...
    if Training:
        wandb.init(entity = "dimah", project="VDP_gru_ECG5000_units_{}_epochs_{}_lr_{}".format(units, epochs, lr)) 
        if continue_training:
            saved_model_path = './VDP_gru/vdp_saved_models_units_{}/vdp_gru_epoch_{}/'.format(units, saved_model_epochs) 
        ...
        if (epochs > 1):
            ...
            plt.ylim(0, 1.1)
            plt.title("VDP GRU on ECG Data")
            plt.xlabel("Epochs")
            plt.ylabel("Accuracy")
            plt.legend(loc='lower right')
            plt.savefig(PATH + 'VDP_gru_on_ECG_Data_acc.png')
            plt.close(fig)
            ...
            plt.ylabel("Error")
            plt.legend(loc='lower right')
            plt.savefig(PATH + 'VDP_gru_on_ECG_Data_error.png')
            plt.close(fig)
