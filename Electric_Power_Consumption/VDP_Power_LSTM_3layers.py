...
class Density_prop_LSTM(tf.keras.Model):
  def __init__(self, units, name=None): 
   ...
   # self.drop_prop = drop_prop
    ...
    self.rnn1 = tf.keras.layers.RNN(self.cell1, return_state=True, return_sequences=True, stateful=True)
    self.rnn2 = tf.keras.layers.RNN(self.cell2, return_state=True, return_sequences=True, stateful=True)
    self.rnn3 = tf.keras.layers.RNN(self.cell3, return_state=True, return_sequences=False, stateful=True)    
...
def main_function(time_step=1, input_dim=7, units=100, output_size=1, batch_size=14, epochs=200, lr=0.0001, lr_end = 0.00001, kl_factor=0.0001,
    ...
    PATH = './exp3/saved_models_with_hidden_unit_{}_lr_{}/VDP_lstm_epoch_{}/'.format(units, lr, epochs)   
    ...
    df_resample.shape    
    ## * Note: we scale all features in range of [0,1].    
...
#                tf.print(v.name, tf.reduce_mean(g))          
    ...
    if Training:
     #   wandb.init(entity = "", project="VDP_LSTM_power_timeseries_units_{}_epochs_{}_lr_{}_3lstm2_exp2_no_drop".format(units, epochs, lr)) 
        if continue_training:
            saved_model_path = './saved_models_with_hidden_unit_{}_lr_{}/VDP_lstm_epoch_{}/'.format(units,lr,  saved_model_epochs)
        ...
        for epoch in range(epochs):
            ...
            for step, (x, y) in enumerate(tr_dataset):
                ...
                if (x.shape[0] == batch_size):                    
                   ...
                   # test_loss.update_state(loss)                
                    ...
                    rmse1 += rmse_err
                    x1 = (x.numpy()).reshape((x.shape[0], 7))                    
                    yhat = (logits.numpy()).reshape((len(y.numpy()), 1))
                    inv_yhat = np.concatenate((yhat, x1[:, -6:]), axis=1)
                    inv_yhat = scaler.inverse_transform(inv_yhat)
                    inv_yhat = inv_yhat[:,0]
                    # invert scaling for actual
                    y1 = (y.numpy()).reshape((len(y.numpy()), 1))
                    inv_y = np.concatenate((y1, x1[:, -6:]), axis=1)
                    inv_y = scaler.inverse_transform(inv_y)
                    inv_y = inv_y[:,0]
                    # calculate RMSE
                    rmse_err1 = np.sqrt(mean_squared_error(inv_y, inv_yhat))
                    rmse2  += rmse_err1
                    
                    tr_no_steps += 1
                    if step % 1000 == 0:                        
                      #  print("\n Step:", step, "Loss:", float(err1 / (tr_no_steps )))
                      #  print("Scaled Training RMSE: %.6f" % float(rmse1 / (tr_no_steps ))) 
                        print("Training RMSE: %.6f" % float(rmse2 / (tr_no_steps )))                        
                   # wandb.log({"Total Training Loss": loss.numpy(),
                   #            "Scaled Training RMSE per minibatch": rmse_err, 
                   #            "Training RMSE per minibatch": rmse_err1,   
                   ...
                   #            'epoch': epoch
                   # })           
            train_rmse[epoch] = rmse2 / tr_no_steps 
            Scaled_train_rmse[epoch] = rmse1 / tr_no_steps 
            train_err[epoch] = err1 / tr_no_steps
            print('Training RMSE  ', train_rmse[epoch])
            print('Scaled Training RMSE  ', Scaled_train_rmse[epoch])
            ...
            for step, (x, y) in enumerate(val_dataset):
                ...
                if (x.shape[0] == batch_size):                    
                    ...
                    inv_y = inv_y[:,0]
                    # calculate RMSE
                    valid_rmse_err1 = np.sqrt(mean_squared_error(inv_y, inv_yhat))
                    rmse_valid2  += valid_rmse_err1                   
                    if step % 1000 == 0:
                       # print("Step:", step, "Loss:", float(total_vloss.numpy()))
                      #  print("Scaled validation RMSE so far: %.6f" % valid_rmse_err)
                        print(" validation RMSE so far: %.6f" % valid_rmse_err1)
                    va_no_steps += 1
                   # wandb.log({"Total Validation Loss": total_vloss.numpy(),
                   #            "Scaled Validation RMSE per minibatch": valid_rmse_err ,
                   #            "Validation RMSE per minibatch": valid_rmse_err1,                               
            ...
            #            "Scaled Training RMSE": (rmse1 / tr_no_steps),   
            #            "Training RMSE": (rmse2 / tr_no_steps),                      
            #            "Validation Loss": (err_valid1 / va_no_steps),
            #            "Validation RMSE": (rmse_valid2 / va_no_steps),
            #            "Scaled Validation RMSE": (rmse_valid1 / va_no_steps),
            #            'epoch': epoch
            #           })
            Scaled_valid_rmse[epoch] = rmse_valid1 / va_no_steps
            valid_rmse[epoch] = rmse_valid2 / va_no_steps
            ...
            print('Scaled Training RMSE  ', Scaled_train_rmse[epoch])
            print('Scaled Validation RMSE  ', Scaled_valid_rmse[epoch])
            print('------------------------------------')
            print('Training Loss  ', train_err[epoch])
    ...
    if(Testing):
        ...
        if Random_noise:
            test_path = 'test_results_random_noise_{}_latest/'.format(gaussain_noise_std)               
        os.makedirs(PATH + test_path)  
        lstm_model.load_weights(PATH + 'vdp_lstm_model')
        test_no_steps = 0        
        rmse_test = np.zeros(int(test_X.shape[0] /batch_size))
        rmse_test_non_scaled = np.zeros(int(test_X.shape[0] /batch_size))
        ...
        inv_yhat_1 = np.zeros([int(test_X.shape[0] / (batch_size)), batch_size])
        inv_y_1 = np.zeros([int(test_X.shape[0] / (batch_size)), batch_size])
        ...
        for step, (x, y) in enumerate(val_dataset):
            ...
            if (x.shape[0] == batch_size):            
                ...
                inv_yhat = scaler.inverse_transform(inv_yhat)
                inv_yhat = inv_yhat[:,0]
                inv_yhat_1[test_no_steps,:] = inv_yhat
                # invert scaling for actual
                y1 = (y.numpy()).reshape((len(y.numpy()), 1))
                inv_y = np.concatenate((y1, x1[:, -6:]), axis=1)
                inv_y = scaler.inverse_transform(inv_y)
                inv_y = inv_y[:,0]
                inv_y_1[test_no_steps,:] = inv_y
                # calculate RMSE
                valid_rmse_err1 = np.sqrt(mean_squared_error(inv_y, inv_yhat))
                rmse_test_non_scaled[test_no_steps] = valid_rmse_err1 
                    
                if step % 100 == 0:
                    print("Test RMSE: %.6f" % valid_rmse_err1 )             
                test_no_steps+=1       
        test_stop = timeit.default_timer()
        print('Total Test Time: ', test_stop - test_start)  
        test_rmse = np.mean(rmse_test)        
        test_rmse_non_scaled = np.mean(rmse_test_non_scaled)
        print('Average Scaled Test RMSE : ', test_rmse )        
        print('Average Test RMSE : ', test_rmse_non_scaled )        
        
        pf = open(PATH + test_path + 'Uncertainty_info.pkl', 'wb')         
        pickle.dump([logits_ ,true_y, inv_yhat_1,inv_y_1, sigma_ , rmse_test_non_scaled, rmse_test], pf)                                          
        pf.close()        
...
#               var[i] = s
        ...
        textfile.write("\n Scaled Test RMSE : "+ str( test_rmse)) 
        textfile.write("\n Test RMSE : "+ str( test_rmse_non_scaled))   
        textfile.write("\n Output Variance: "+ str(np.mean(sigma_)))    
      #  textfile.write("\n Trimmed mean of the Output Variance: "+ str(stats.trim_mean(np.abs(sigma_), 0.011, axis=None) ))              
        textfile.write("\n---------------------------------")
        if Random_noise:
    ...
    if(Adversarial_noise):        
        test_path = 'test_results_adversarial_noise_{}_latest/'.format(epsilon)  
        os.makedirs(PATH + test_path)  
        lstm_model.load_weights(PATH + 'vdp_lstm_model')
        lstm_model.trainable = False        
        ...
        for step, (x, y) in enumerate(val_dataset):
            ...
            if (x.shape[0] == batch_size):                                 
               ...
               # print(adv_perturbations[test_no_steps, :, :, :])
                ...
                #print("adv_x", np.mean(adv_x)) 
                
                 
            #    print('x', np.mean(x))  
                logits, sigma = test_on_batch(adv_x)  
               # print("logits", np.mean(logits))
              #  print("sigma", np.mean(sigma))
                ...
                inv_y = inv_y[:,0]
                inv_y_1[test_no_steps,:] = inv_y
                #print("inv_y", np.mean(inv_y))
               # print("inv_yhat",   np.mean(inv_yhat))
                # calculate RMSE
                valid_rmse_err1 = np.sqrt(mean_squared_error(inv_y, inv_yhat))
                rmse_test_non_scaled[test_no_steps] = valid_rmse_err1  
...
#        print('Trimmed both of the Output Variance', np.mean(stats.trimboth(np.abs(sigma_1), 0.01, axis=None)) )           
    ...
    if(BIM_adversarial):        
        test_path = 'test_results_BIMadversarial_noise_{}_alpha{}_iter{}/'.format(epsilon, alpha, maxAdvStep)  
        os.makedirs(PATH + test_path)   
        lstm_model.load_weights(PATH + 'vdp_lstm_model')
        lstm_model.trainable = False        
...
#        print('Trimmed both of the Output Variance', np.mean(stats.trimboth(np.abs(sigma_1), 0.01, axis=None)) )           
        ...
        textfile.write('\n KL term factor : ' +str(kl_factor))       
        textfile.write("\n---------------------------------")
        textfile.write('\n Total test time in sec with BIM attack : ' +str(bim_adv_test_stop - bim_adv_test_start))   
        textfile.write("\n Scaled Test RMSE : "+ str( test_rmse)) 
        textfile.write("\n Test RMSE : "+ str( test_rmse_non_scaled))   
        textfile.write("\n Output Variance: "+ str(np.mean(np.abs(sigma_)))   )                    
        textfile.write("\n---------------------------------")
